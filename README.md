...
## <a name="test"> 4. 테스트 코드 </a>
이전부터 테스트코드는 제가 반드시 잘 작성해보고 싶은 파트중 하나였습니다. 

제가 만든 로직에 버그를 효율적으로 찾아 더 좋은 로직으로 변경하고싶었기에 좋은 테스트코드를 작성하고싶었습니다. 

Jest의 공식문서를 살펴보니 테스트코드를 작성하는 과정(문법 등)이 어려운 것이 아니라고 느꼈고 "계획을 어떻게 새워야할까?" 가 고민의 포인트였습니다. 

![스크린샷 2023-07-21 오후 6 48 42](https://github.com/Ukeydock/NestJS/assets/71562311/a4389b45-d1fe-46ac-8fc0-6ea2806b2110)

테스트코드의 폴더 구조는 이와 같습니다. 

테스트용 모듈, 테스트용 데이터 등을 생성하는 파일을 만들어두고(test.module.ts) 여러 테스트파일에서 이 기반 파일에 의존합니다. 

데이터베이스와 통신하는 repository 안의 메소드들은 테스트용 데이터베이스를 생성해 테스트합니다. 연결되어있는 Docker Mysql 컨테이너에서 진행합니다.

비즈니스 로직을 담당하는 service에서는 repository안의 메소드들이 성공했다는 가정 하에 의존하고있는 메소드들을 모킹합니다. 
모킹의 의도가 의존하고 있는 모듈의 반환값을 임의로 조정하는 것에 있는만큼 의존하고 있는 모듈이 문제가 없다면 모킹을 사용해도 괜찮겠다고 판단했습니다.

controller의 경우 따로 단위테스트를 진행하지 않고 e2e테스트로 대체할 수 있다고 판단했습니다. 테스트용 데이터베이스, 테스트용 데이터를 구축해놓고 실제로 요청이 들어오는 것처럼 테스트를 진행합니다. 

기본적인 테스트용 데이터를 테스트 실행시마다 실행시켜 테스트끼리의 독립성을 확보하였습니다. 

블로그에 테스트 코드 관련 게시물을 작성하고 있습니다. 

[벨로그: 나도 테스트코드를 작성해보겠어](https://velog.io/@kwanyung/%EB%82%98%EB%8F%84-%ED%85%8C%EC%8A%A4%ED%8A%B8%EC%BD%94%EB%93%9C%EB%A5%BC-%EC%9E%91%EC%84%B1%ED%95%B4%EB%B3%B4%EA%B2%A0%EC%96%B4-1)


![스크린샷 2023-07-22 오후 6 12 13](https://github.com/Ukeydock/NestJS/assets/71562311/f69e2ad8-bdc2-4736-b320-01a48e2b7959)

7월 22일기준, 테스트 커버리지를 측정했을 때 부족한 부분이 있지만 지속적으로 개선해나가고 있습니다. 



---

## <a name="question"> 5. 고민한 부분</a>
